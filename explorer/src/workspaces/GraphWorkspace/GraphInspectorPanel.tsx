  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 8, marginTop: 8 }}>
      ...
      <div style={{ display: "flex", gap: 6, flexWrap: "wrap", alignItems: "center" }}>
        ...
        >
          {result.distance_band} · {result.hop_count} salto{result.hop_count !== 1 ? "s" : ""}
        </span>
        {(result.alternative_path_count ?? 0) > 0 && (
          <span style={subtleChipStyle}>{result.alternative_path_count} camino{result.alternative_path_count !== 1 ? "s" : ""} alt.</span>
        )}
      </div>
      ...
      {hasMetrics && <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
        {result.confidence_decay != null && (
          <div style={metricCardStyle}>
            <div style={metricLabelStyle}>Decaimiento de confianza</div>
            <div
              style={{
                ...metricValueStyle,
        ...
        {result.semantic_similarity != null && (
          <div style={metricCardStyle}>
            <div style={metricLabelStyle}>Sim. semántica</div>
        ...
        {result.path_coherence_score != null && (
          <div style={metricCardStyle}>
            <div style={metricLabelStyle}>Coherencia de camino</div>
        ...
        {result.bottleneck_node && (
          <div style={metricCardStyle}>
            <div style={metricLabelStyle}>Cuello de botella</div>
            <div
              style={{
                ...metricValueStyle,
                color: "#e3b341",
...
function PathFlowViz({
  ...
  if (path.length === 0) {
    return <div style={emptyTextStyle}>No se encontró camino entre los nodos seleccionados.</div>;
  }
      ...
      <aside style={{ padding: 24, display: "flex", flexDirection: "column", gap: 16 }}>
        <div style={{ borderBottom: `1px solid ${GRAPH_THEME.ui.surface.divider}`, paddingBottom: 16 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
            ...
            <span style={{ color: GRAPH_THEME.ui.timeline.playhead, fontSize: 12, fontWeight: 700 }}>Selección</span>
          </div>
        ...
        <div style={groupedSelectionNoticeStyle}>
          <div style={{ color: GRAPH_THEME.ui.text.strong, fontWeight: 600, marginBottom: 6 }}>El elemento seleccionado no es directamente inspeccionable en el grafo actual.</div>
          <div style={{ color: GRAPH_THEME.ui.text.body, fontSize: 13, lineHeight: 1.6 }}>
            {canActivateFocused
              ? "Activa el modo Enfoque para resolver esta selección agrupada a su nodo canónico."
              : (focusedUnavailableReason ?? "El modo Enfoque no está disponible para la selección actual.")}
          </div>
        </div>
      </aside>
    );
    ...
    <aside style={{ padding: 24, display: "flex", flexDirection: "column", gap: 18 }}>
      ...
      <div style={{ borderBottom: `1px solid ${GRAPH_THEME.ui.surface.divider}`, paddingBottom: 16 }}>
        ...
        {groupedDisplaySelection ? (
          <div style={groupedSelectionNoticeStyle}>
            <div style={{ color: GRAPH_THEME.ui.text.strong, fontWeight: 600, marginBottom: 6 }}>Este elemento agrupado permanece a nivel de visualización hasta que actives explícitamente el modo Enfoque.</div>
            <div style={{ color: GRAPH_THEME.ui.text.body, fontSize: 13, lineHeight: 1.6 }}>
              {canActivateFocused
                ? `Nodo canónico disponible: ${effectiveNodeId}`
                : (focusedUnavailableReason ?? "El modo Enfoque no está disponible para la selección actual.")}
            </div>
          </div>
        ) : null}
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 12 }}>
          ...
          {attribution.length ? <span style={subtleChipStyle}>{attribution.length} campos de origen</span> : null}
          {predictions.length ? <span style={subtleChipStyle}>{predictions.length} enlaces candidatos</span> : null}
        </div>
      </div>
      ...
      {(attributes?.valid_from || attributes?.valid_until) ? (
        <div style={{ padding: "10px 12px", background: "rgba(233, 196, 122, 0.075)", border: "1px solid rgba(233, 196, 122, 0.22)", borderRadius: 8, fontSize: 12, color: GRAPH_THEME.palette.accent.selected, fontFamily: "monospace" }}>
          {attributes?.valid_from ? <div>desde: {attributes.valid_from}</div> : null}
          {attributes?.valid_until ? <div>hasta: {attributes.valid_until}</div> : null}
        </div>
      ) : null}

      {/* Actions */}
      <section style={sectionStyle}>
        <div style={sectionTitleStyle}>Acciones</div>
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          ...
          >
            ...
            {isRunningPredictions ? "Ejecutando…" : "Predecir enlaces"}
          </button>
      ...
      {/* Trace Path */}
      <section style={sectionStyle}>
        <div style={sectionTitleStyle}>Trazar camino</div>
        <input
          value={pathTargetId}
          ...
          style={inputStyle}
        />
        <button style={actionButtonStyle} onClick={onTracePath} disabled={!actionNodeId}>Trazar camino causal</button>
        ...
        {pathResult?.path?.length ? (
          ...
          <div style={emptyTextStyle}>
            Elige un destino o haz clic en una predicción candidata para preparar un trazado de camino.
          </div>
        )}
      </section>
      ...
      <details className="node-panel-collapse" open={predictions.length > 0}>
        <summary className="node-panel-summary">Enlaces candidatos</summary>
        <div className="node-panel-body">
          {predictions.length > 0 ? (
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8, padding: 8, color: "#8b949e", fontSize: 12 }}>
              ...
              <span>Calculando enlaces candidatos…</span>
            </div>
          ) : (
            <div style={emptyTextStyle}>Ejecuta la predicción de enlaces para descubrir relaciones probables.</div>
          )}
        </div>
      </details>
      ...
      <details className="node-panel-collapse">
        <summary className="node-panel-summary">Atribución de origen</summary>
        <div className="node-panel-body">
          {attribution.length ? (
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            ...
            <div style={emptyTextStyle}>No se encontró metadata de atribución explícita en este nodo.</div>
          )}
        </div>
      </details>
      ...
      {/* Properties */}
      <details className="node-panel-collapse">
        <summary className="node-panel-summary">Propiedades</summary>
        <div className="node-panel-body">
          {propertyEntries.length ? (
            ...
            <div style={emptyTextStyle}>No hay propiedades adicionales adjuntas a este nodo.</div>
          )}
        </div>
      </details>
    </aside>
